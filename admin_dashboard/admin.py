...
class PelangganAdmin(BaseModelAdmin):
    ...
    def set_birthday_discount_for_loyal_customers(self, request, queryset):
        ...
        for pelanggan in queryset:
            ...
            pesan = "Selamat ulang tahun! Diskon 10% untuk Anda, silakan belanja agar diskon ini tidak hilang."
...
class TransaksiAdmin(BaseModelAdmin):
    list_display = ['nomor', 'idPelanggan', 'tanggal', 'status_transaksi_interactive', 'ongkir', 'bukti_bayar_display', 'combined_actions']
    list_filter = ['status_transaksi']
    search_fields = ['idPelanggan__nama_pelanggan']
...
class NotifikasiAdmin(BaseModelAdmin):
    list_display = ['idPelanggan', 'tipe_pesan', 'is_read', 'created_at', 'get_actions_links']
    search_fields = ['idPelanggan__nama_pelanggan', 'tipe_pesan']
